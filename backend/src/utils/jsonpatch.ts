  ...
  for (const operation of patch) {
    ...
    if (operation.op === 'add' || operation.op === 'replace') {
      if (!('value' in operation)) {
        throw new Error(`Operation "${operation.op}" requires a "value" field`)
      }
    }
  }
}
...
export function applyEvents(baseState: SeedState, events: EventData[]): SeedState {
  ...
  let state: SeedState = {
    ...
    tags: baseState.tags ? [...baseState.tags] : undefined,
    categories: baseState.categories ? [...baseState.categories] : undefined,
