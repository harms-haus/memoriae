...
cd "$MOTHER_DIR"
npm run build || {
  echo "❌ Mother-theme build failed"
  exit 1
}

# Verify mother-theme build output
if [ ! -d "dist" ] || [ -z "$(ls -A dist 2>/dev/null)" ]; then
  echo "❌ Error: Mother-theme build output directory 'dist' is empty or missing"
  exit 1
fi
echo "✅ Mother-theme build successful"
